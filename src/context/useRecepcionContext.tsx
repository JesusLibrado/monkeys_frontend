...
export const RecepcionContext = createContext<RecepcionContextType | undefined>(
  undefined,
);
...
export const useRecepcionContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useRecepcionContext can only be used within RecepcionProvider",
    );
  }
  return context;
};
...
export const RecepcionProvider = (props: { children: any }) => {
  ...
  const [folioFactura, setFolioFactura] = useState<Map<number, string>>(
    new Map(),
  );
