...
const ConceptosFacturaEditor = (props: { facturaId: string }) => {
  ...
  const [
    ...
    { loading: removing, error: removeError, data: deletedConcepto },
  ] = useMutation(REMOVE_CONCEPTO_FROM_FACTURA);
  ...
  const getTotalFactura = () =>
    (conceptosFacturaData || []).reduce((acc, current) => {
        ...
        <table className="table text-center table-nowrap align-middle mb-0 table-sm">
          ...
          <tbody id="products-list">
            <AgregarConceptoFactura
              ...
              addConceptoFactura={addConceptoFactura}
            />
