...
const MenuItemLink = ({ item, className }: SubMenus) => {
  ...
  return (
    <Link href={item.url ?? ''} target={item.target} onClick={ToggleMenu} className={clsx(className, { disabled: item.isDisabled })}>
      {item.icon && (
        <span className="menu-icon">
          <IconifyIcon icon={item.icon} />
        </span>
            ...
            {item.isTitle ? (
              ...
              <>
                {item.children ? (
                  ...
                  <MenuItem item={item} toggleMenu={toggleMenu} level={1} linkClassName={clsx('side-nav-link', getActiveClass(item))} className={clsx('side-nav-item', getActiveClass(item))} />
                )}
              </>
            )}
          </Fragment>
