...
const LeftSideBarToggle = () => {
  const { mainMenu, changeMainMenuSize, toggleBackdrop, orientation,horizontalMenu } = useLayoutContext()
  ...
  const handleMenuSize = () => {
    ...
    if (mainMenu.size === 'compact') changeMainMenuSize('condensed')
    else if (mainMenu.size === 'default') changeMainMenuSize('condensed')
  }
    ...
    <>
      {
        orientation == 'horizontal' ?
          <button onClick={horizontalMenu.toggle} className="sidenav-toggle-button px-2">
