...
const serviciosByCategoria = (servicios: any[]) =>
  servicios.reduce((serviciosByCategoria, currentValue) => {
    ...
    if (serviciosByCategoria.has(categoria)) {
      ...
      serviciosByCategoria.set(categoria, [
        ...currentAccumulated,
        currentValue,
      ]);
    } else {
...
const getPrecio = (concepto: any) => {
  if (concepto.servicio) return concepto.servicio.precio;
  if (concepto.producto) return concepto.producto.precioPublico;
  else return 0;
};
...
const SelectConcepto = (props: { onSelectedConceptoChange: Function }) => {
  let {
    loading: loadingProductos,
    error: errorProductos,
    data: productosData,
  } = useQuery(GET_AVAILABLE_PRODUCTOS);
  ...
  let {
    ...
    data: serviciosData,
  } = useQuery(GET_AVAILABLE_SERVICIOS);
